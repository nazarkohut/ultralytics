        ...
        if not self.is_fused():
            for m in self.model.modules():
                ...
                if isinstance(m, ConvTranspose) and hasattr(m, "bn"):
                    ...
                    delattr(m, "bn")  # remove batchnorm
                    m.forward = m.forward_fuse  # update forward
                if isinstance(m, RepConv): # TODO: Have no understanding whether this one works at all as it is not displayed in onnx(even if fuse is commented)
                    m.fuse_convs()
                    m.forward = m.forward_fuse  # update forward
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m is C3k2:  # for M/L/X sizes
                ...
                if scale in "mlx":
                    args[3] = True
        elif m in {CBAM, ECA, OriginalCBAM, SIMAM}: # TODO: for SIMAM we do no need parameters, so not sure whether to put it here
            c1, c2 = ch[f], args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, *args[1:]]
